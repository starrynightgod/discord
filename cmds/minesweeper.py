...
class minesweeper(commands.Cog):
    ...
    @commands.command(name='minesweeper', aliases=['踩地雷' , 'ms'], brief="game", description=f"踩地雷遊戲\n使用方法：\n{jdata['command_prefix']}ms <列> <行> <炸彈>\n你可以不要給予值 直接使用 會給你隨機列、行、炸彈的數量唷。")
    async def minesweeper(self, ctx, columns = None, rows = None, bombs = None):
        if columns is None or rows is None and bombs is None:
            if columns is not None or rows is not None or bombs is not None:
                ...
                columns = random.randint(4,14)
                rows = random.randint(4,14)
                bombs = columns * rows - 1
                bombs = bombs / 2.5
                bombs = round(random.randint(5, round(bombs)))
            else:
                #若不給參數的話 會給4到14範圍 隨機給行列的數量
                #（（列×行）－1）÷ 2.5
                #這是確保我們網格上的炸彈比例不會太高
                columns = random.randint(4,20)
                rows = random.randint(4,20)
                bombs = columns * rows - 1
                bombs = bombs / 2.5
                bombs = round(random.randint(5, round(bombs)))
        try:
            ...
            await ctx.send(errortxt)
            return
        if columns > 14 or rows > 14:
            await ctx.send('由於Discord限制 列和行的限制為14')
            return
        ...
        string_builder = '\n'.join(string_builder)
        # 替換數字和表情對應
        string_builder = string_builder.replace('0', '||０||')
        string_builder = string_builder.replace('1', '||１||')
        string_builder = string_builder.replace('2', '||２||')
        string_builder = string_builder.replace('3', '||３||')
        string_builder = string_builder.replace('4', '||４||')
        string_builder = string_builder.replace('5', '||５||')
        string_builder = string_builder.replace('6', '||６||')
        string_builder = string_builder.replace('7', '||７||')
        string_builder = string_builder.replace('8', '||８||')
        final = string_builder.replace('B', '||轟||')
        ...
        embed.add_field(name='炸彈[轟] 數量：', value=bombs, inline=True)
        embed.add_field(name='炸彈[轟] 百分比：', value=f'{percentage}%', inline=True)
