...
class wiki(Cog_Extension):
  @commands.command(name='wiki',aliases=['維基'], brief="warframe", description=f"查詢warframe wiki\n使用方法：{jdata['command_prefix']}wiki [你要查詢的東西(可用繁體)]")
    ...
    try:
      ...
      if page.exists == False:
        ...
        if page.exists == False:
          page = en.pages[name]
          if page.exists == False:
            await ctx.send("頁面不存在，𝑺𝒂𝒌𝒖𝒓𝒂❀在等待指揮官為Wiki作出貢獻呢")
            return
          else: 
